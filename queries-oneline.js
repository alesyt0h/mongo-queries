...
db.restaurants.find( { $and: [ { 'grades.score': { $gt: 80 } }, { 'grades.score': { $not: { $gt: 100 } } }, ] } ).pretty();
...
db.restaurants.find( { $and: [ { cuisine: { $ne: 'American ' } }, { borough: { $ne: 'Brooklyn' } }, { 'grades.grade': 'A' }, ] } ).sort({cuisine: -1}).pretty();
...
db.restaurants.find( { $or: [ { borough: 'Staten Island' }, { borough: 'Queens' }, { borough: 'Bronx' }, { borough: 'Brooklyn' }, ] }, { restaurant_id: true, name: true, borough: true, cuisine: true } ).pretty();
...
db.restaurants.find( { $and: [ { 'grades.score': { $lt: 10 } }, { 'grades.score': { $not: { $gt: 10 } } } ] }, { restaurant_id: true, name: true, borough: true, cuisine: true } ).pretty();
...
db.restaurants.find( { grades: { $elemMatch: { $and: [ { grade: 'A' }, { score: 11 }, { date: ISODate("2014-08-11T00:00:00Z") } ] } } }, { restaurant_id: true, name: true, grades: true } ).pretty();
...
db.restaurants.find( { $and: [ { 'grades.1.grade': 'A' }, { 'grades.1.score': 9 }, { 'grades.1.date': ISODate("2014-08-11T00:00:00Z") } ] }, { restaurant_id: true, name: true, grades: true } ).pretty();
...
db.restaurants.find( { $and: [ {'address.coord.1': { $gt: 42}}, {'address.coord.1': { $lte: 52}}, ] }, { restaurant_id: true, name: true, address: true, borough: true } ).pretty();

// Escriu una consulta per organitzar el nom dels restaurants en ordre ascendent juntament amb totes les columnes
db.restaurants.aggregate( [ { $sort: { name: 1 } } ] ).pretty();
...
db.restaurants.aggregate( [ { $sort: { cuisine: 1, borough: -1 } } ] ).pretty();
...
db.restaurants.aggregate( [ { $project: { restaurant_id: true, name: true, grades: true, newScores: { $map: { input: '$grades.score', as: 'score', in: { $divide: ['$$score', 7] } } } } }, { $match: { newScores: 0 } }, { $project: { newScores: false } } ] ).pretty();
